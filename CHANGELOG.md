...
- Exposed `PageSpeedAuditMissingError` from the package root and from a new backend-free `screamingfrog.exceptions` module, so a consumer can catch a stale-audit-mapping failure without importing `screamingfrog.backends.derby_backend` (which pulls in the Derby/JPype stack) and without matching on `type(exc).__name__`. The class is unchanged and the existing import path still works.
- Extended the pinned PageSpeed audit-id mapping to the opportunities summary tab, which resolved fifteen audits through its own unpinned dict (`_pagespeed_opportunity_specs`): ten of those ids were outside `PAGESPEED_AUDIT_IDS` entirely, including the three image audits Lighthouse 13 consolidated into "Improve Image Delivery". This tab degraded worse than the per-audit tabs, which go empty: it emits one row per label regardless, so a renamed id produced a complete-looking table of zeros indistinguishable from a fast site. New `PAGESPEED_OPPORTUNITY_AUDIT_IDS` holds the label mapping under the same re-verify-on-upgrade contract, and `_check_pagespeed_audits_seen` raises `PageSpeedAuditMissingError` for any missing pinned id once PSI payloads exist. Presence is counted rather than affectedness, so a passing audit on a fast site is never mistaken for a rename.
