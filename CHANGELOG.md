...
- Extended the pinned PageSpeed audit-id mapping to the opportunities summary tab, which resolved fifteen audits through its own unpinned dict (`_pagespeed_opportunity_specs`): ten of those ids were outside `PAGESPEED_AUDIT_IDS` entirely, including the three image audits Lighthouse 13 consolidated into "Improve Image Delivery". This tab degraded worse than the per-audit tabs, which go empty: it emits one row per label regardless, so a renamed id produced a complete-looking table of zeros indistinguishable from a fast site. New `PAGESPEED_OPPORTUNITY_AUDIT_IDS` holds the label mapping under the same re-verify-on-upgrade contract, and `_check_pagespeed_audits_seen` grades its response: no pinned id present in any payload raises `PageSpeedAuditMissingError` (wholly stale mapping), while a partial miss warns with every missing id named, since Screaming Frog lets the operator choose which PSI opportunities to store and a partial `audits` object cannot be ruled out from the backend. Presence is counted rather than affectedness, so a passing audit on a fast site is never mistaken for a rename.
