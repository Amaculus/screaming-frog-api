...
def register_structured_data_filters() -> None:
    filters = [
        ...
        FilterDef(
            ...
            sql_where="SERIALISED_STRUCTURED_DATA IS NOT NULL AND LENGTH(SERIALISED_STRUCTURED_DATA) > 0",
        ),
        FilterDef(
            name="Missing",
            tab="Structured Data",
            description="Missing structured data.",
            sql_where="SERIALISED_STRUCTURED_DATA IS NULL OR LENGTH(SERIALISED_STRUCTURED_DATA) = 0",
        ),
        FilterDef(
        ...
        FilterDef(
            name="Parse Errors",
            tab="Structured Data",
            description="URLs with structured data parse errors.",
            sql_where="PARSE_ERROR_MSG IS NOT NULL AND PARSE_ERROR_MSG <> ''",
        ),
        FilterDef(
            name="Microdata URLs",
            tab="Structured Data",
            description="URLs with Microdata structured data.",
            sql_where="LENGTH(SERIALISED_STRUCTURED_DATA) > 0",
        ...
        FilterDef(
            name="JSON-LD URLs",
            tab="Structured Data",
            description="URLs with JSON-LD structured data.",
            sql_where="LENGTH(SERIALISED_STRUCTURED_DATA) > 0",
            blob_column="SERIALISED_STRUCTURED_DATA",
        ...
        FilterDef(
            name="RDFa URLs",
            tab="Structured Data",
            description="URLs with RDFa structured data.",
            sql_where="LENGTH(SERIALISED_STRUCTURED_DATA) > 0",
            blob_column="SERIALISED_STRUCTURED_DATA",
            blob_pattern=b"RDFA",
        ),
