...
def _measure_text_pixels_tk(text: str, *, family: str, size: int, weight: str) -> int | None:
    ...
    try:
        if _TK_ROOT is None:
            ...
            # After Tk is initialized, change the NSApplication activation policy to
            # NSApplicationActivationPolicyAccessory (1): app becomes invisible in the
            # Dock and cannot be brought to the front as a regular app.
            # Must be set AFTER tk.Tk() to avoid crashing Tk's NSApp initialisation.
            try:
                import ctypes, ctypes.util, platform
                if platform.system() == "Darwin":
                    _lib = ctypes.cdll.LoadLibrary(ctypes.util.find_library("objc") or "libobjc.A.dylib")
                    ...
                    _SharedFn = ctypes.CFUNCTYPE(ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p)
                    _PolicyFn = ctypes.CFUNCTYPE(ctypes.c_bool, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_long)
                    _msg_ptr = ctypes.cast(_lib.objc_msgSend, ctypes.c_void_p).value
                    _cls = _lib.objc_getClass(b"NSApplication")
                    _nsapp = _SharedFn(_msg_ptr)(_cls, _lib.sel_registerName(b"sharedApplication"))
                    _PolicyFn(_msg_ptr)(_nsapp, _lib.sel_registerName(b"setActivationPolicy:"), ctypes.c_long(1))
            except Exception:
                pass
            _TK_ROOT = root
